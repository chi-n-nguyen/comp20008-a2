...
def create_wfh_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convert to numeric (handle 'Yes'/'No' or similar)
    for col in existing_weekdays:
        ...
        # Map to binary
        df_features[col] = df_features[col].map({'Yes': 1, 'No': 0, 'Y': 1, 'N': 0})
...
def create_household_wfh_metrics(persons_df: pd.DataFrame, households_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Create household WFH saturation metric
    hh_wfh['hh_wfh_saturation'] = hh_wfh['total_wfh_adopters'] / (hh_wfh['total_workers'] + 0.001)
